...
const updateName = (req, res) => {
    ...
    petFound.nomeFantasia = nomeFantasiaRequest // atualiza o objeto com novo nome
    ...
    fs.writeFile("./src/models/pets.json", JSON.stringify(pets), 'utf8', function (err) { // gravando novo pet no array de pets
        if (err) {
            ...
            const petUpdated = pets.find(pet => pet.id == idPetRequest) //separo o pet que modifiquei no array
...
const petService = (req, res) => {
    try {
      ...
      if (filteredPet.length > 0) {res.status(200).send(filteredPet)

      } else {res.status(404).send({ message: "Petshop not found!" })}
